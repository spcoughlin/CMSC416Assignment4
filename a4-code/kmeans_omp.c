...
int main(int argc, char **argv) {
    ...
    // initial assignment: distribute points round-robin
    for (int i = 0; i < data.ndata; i++) {
    ...
    while (nchanges > 0 && curiter <= MAXITER) {
        ...
        #pragma omp parallel
        {
            // calloc gives us a zeroed private array
            float *priv_feat = (float *)calloc((size_t)nclust * (size_t)clust.dim, sizeof(float));
            int *priv_cnt = (int *)calloc((size_t)nclust, sizeof(int));

            if (!priv_feat || !priv_cnt) {
                fprintf(stderr, "Thread-private alloc failed\n");
                exit(1);
            }

            // each thread gets a contiguous chunk of data points (schedule static)
            #pragma omp for schedule(static)
            for (int i = 0; i < data.ndata; i++) {
                int c = data.assigns[i];
                for (int d = 0; d < clust.dim; d++) {
                    priv_feat[(size_t)c * (size_t)clust.dim + (size_t)d] +=
                        data.features[(size_t)i * (size_t)data.dim + (size_t)d];
                }
                priv_cnt[c]++;
            }
            // merge this thread's partial sums into the shared global arrays
            #pragma omp critical
            {
                for (int c = 0; c < nclust; c++) {
                    clust.counts[c] += priv_cnt[c];
                    for (int d = 0; d < clust.dim; d++) {
        ...
        // Phase 2
        #pragma omp parallel for schedule(static) reduction(+:nchanges)
        for (int i = 0; i < data.ndata; i++) {
            int best_clust = 0;
